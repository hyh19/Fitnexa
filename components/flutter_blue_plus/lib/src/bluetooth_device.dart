...
class BluetoothDevice {
  ...
  Future<void> connect({
    ...
    if (timeout == null) {
      ...
      await state
          .firstWhere((s){
            print("connect##########:${s.toString()}");
            if(s == BluetoothDeviceState.connected){
              return true;
            }else if(s == BluetoothDeviceState.disconnected){
              throw TimeoutException('Failed to connect', timeout);
            }else{
              return false;
            }
          })
  ...
  Future<void> dissPar() async {
    if (Platform.isAndroid) {
      return FlutterBluePlus.instance._channel
          .invokeMethod('dissPar', id.toString());
    }
  ...
  Future<void> createSocket() async {
    return FlutterBluePlus.instance._channel
        .invokeMethod('createSocket', id.toString());
  }
