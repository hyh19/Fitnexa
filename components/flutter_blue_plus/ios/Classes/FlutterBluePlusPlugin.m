...
@property(nonatomic, strong) FlutterBluePlusStreamHandler *stateStreamHandler;
@property(nonatomic, strong) CBCentralManager *centralManager;
@property(nonatomic) NSMutableDictionary *scannedPeripherals;
@property(nonatomic) NSMutableArray *servicesThatNeedDiscovered;
@property(nonatomic) NSMutableArray *characteristicsThatNeedDiscovered;
@property(nonatomic) LogLevel logLevel;
@property(nonatomic) BOOL isConnectStat;
@property(nonatomic) CBManagerState UpCBManagerState;
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if ([@"state" isEqualToString:call.method]) {
        ...
        if(ProtosBluetoothState_State_On == [self toBluetoothStateProto:self->_centralManager.state].state) {
            result(@(YES));
        } else {
            result(@(NO));
        }
        ...
        @try {
            ...
            [_channel invokeMethod:@"blueLog" arguments:[NSString stringWithFormat:@"connectPeripheral:%@", [peripheral identifier]]];
            
            self.isConnectStat = YES;
            NSLog(@"come3");
        ...
        @try {
            [_channel invokeMethod:@"blueLog" arguments:@"cancelPeripheralConnection"];
        ...
        NSString *remoteId = [call arguments];
        @try {
            if(self.isConnectStat){
                CBPeripheral *peripheral = [self findPeripheral:remoteId];
                result([self toFlutterData:[self toDeviceStateProto:peripheral state:CBPeripheralStateConnecting]]);
            }else{
                CBPeripheral *peripheral = [self findPeripheral:remoteId];
    ...
    }else if([@"blueLog" isEqualToString:call.method]) {
        @try {
            result(@"");
        } @catch(NSException *e) {
            result(e.description);
...
- (CBPeripheral*)findPeripheral:(NSString*)remoteId {
    ...
    if(peripheral == nil) {
        [_channel invokeMethod:@"blueLog" arguments:[FlutterError errorWithCode:@"locateCharacteristic"
...
- (CBDescriptor*)locateDescriptor:(NSString*)descriptorId characteristic:(CBCharacteristic*)characteristic {
    ...
    if(descriptor == nil) {
        [_channel invokeMethod:@"blueLog" arguments:[FlutterError errorWithCode:@"locateDescriptor"
...
- (void)centralManagerDidUpdateState:(nonnull CBCentralManager *)central {
    ...
    NSLog(@"蓝牙开启状态%ld--%.0f",(long)central.state,[date timeIntervalSince1970]*1000);
    [_channel invokeMethod:@"blueLog" arguments:[NSString stringWithFormat:@"蓝牙开启状态%ld--%.0f",(long)central.state,[date timeIntervalSince1970]*1000]];
    ...
    //        if(self.UpCBManagerState != CBManagerStatePoweredOn && central.state == CBManagerStatePoweredOn && self.cBPeripheral != nil){
    //            [self.centralManager stopScan];
    //            [self.centralManager  cancelPeripheralConnection:self.cBPeripheral];
    //            self.centralManager = nil;
    //            self.centralManager = [[CBCentralManager alloc] initWithDelegate:self queue:nil];
    //            NSLog(@"重新初始化蓝牙中心2");
    //            [_channel invokeMethod:@"blueLog" arguments:@"重新初始化蓝牙中心"];
    //        }else{
    //            NSLog(@"11111111111%ld--%.0f",(long)central.state,[date timeIntervalSince1970]*1000);
    //            [_channel invokeMethod:@"blueLog" arguments:[NSString stringWithFormat:@"11111111111%ld--%.0f",(long)central.state,[date timeIntervalSince1970]*1000]];
    //          if(_stateStreamHandler.sink != nil) {
    //            FlutterStandardTypedData *data = [self toFlutterData:[self toBluetoothStateProto:self->_centralManager.state]];
    //            self.stateStreamHandler.sink(data);
    //          }
    //        }
    //        self.UpCBManagerState = central.state;
}
...
- (void)centralManager:(CBCentralManager *)central didConnectPeripheral:(CBPeripheral *)peripheral {
    ...
    self.isConnectStat = NO;
    // Register self as delegate for peripheral
    peripheral.delegate = self;
    [_channel invokeMethod:@"blueLog" arguments:@"蓝牙连接成功"];
...
- (void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    ...
    if(error != NULL){
        ...
        [_channel invokeMethod:@"blueLog" arguments:[NSString stringWithFormat:@"didDisconnectPeripheral:%@",error.localizedDescription]];
    }else{
        [_channel invokeMethod:@"blueLog" arguments:@"didDisconnectPeripheral"];
    }
    // Send connection state
    [_channel invokeMethod:@"DeviceState" arguments:[self toFlutterData:[self toDeviceStateProto:peripheral state:peripheral.state]]];
}
...
- (void)centralManager:(CBCentralManager *)central didFailToConnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error {
    ...
    if(error != NULL){
        NSLog(@"%@", error.localizedDescription);
        [_channel invokeMethod:@"blueLog" arguments:[NSString stringWithFormat:@"didFailToConnectPeripheral:%@",error.localizedDescription]];
    }else{
        [_channel invokeMethod:@"blueLog" arguments:@"didFailToConnectPeripheral"];
    }
    [_channel invokeMethod:@"DeviceState" arguments:[self toFlutterData:[self toDeviceStateProto:peripheral state:peripheral.state]]];
}
