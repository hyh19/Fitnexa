...
class FlutterBluePlus {
  ...
  Stream<String?> get blueLog async* {
    yield await  FlutterBluePlus.instance._channel
        .invokeMethod('blueLog');

    yield* FlutterBluePlus.instance._methodStream
        .where((m) => m.method == "blueLog")
        .map((m) => m.arguments)
        .map((buffer) => buffer);
  }
...
class AdvertisementData {
  ...
  String getNiceManufacturerData() {
    if (manufacturerData.isEmpty) {
      return 'N/A';
    }
    List<String> res = [];
    manufacturerData.forEach((id, bytes) {
      res.add(getNiceHexArray(bytes));
    });
    print('res==$res');
    List<String> split = res.toString().replaceFirst('[', '').replaceFirst(']', '').split(":");
    String s = '';
