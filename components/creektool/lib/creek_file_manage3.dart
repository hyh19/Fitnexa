...
class FileManage3{
  ...
  factory  FileManage3() => _getInstance()!;
  static FileManage3? get instance => _getInstance();
  static FileManage3? _instance;
  IOSink? ioSink;
  String? _fileName;
  File? file;
  List<String> contents = [];
  List<List<int>> listContents = [];
  ///默认是开始状态
  ...
  static FileManage3? _getInstance(){
    _instance ??=  FileManage3._init();
  ...
  ///接收写入数据
  writeReceivedListData(String fileName,List<int> content) async{
    if(_fileName != fileName){
      _fileName = fileName;
      isCreate = true;
      ///开启新文件需要关闭上一个文件
  ...
  _doTask2() async{
    bool isBool = await CompleterExt.awaitFor<bool>((run) async{
      run(await _writeCounter2(listContents.first));
    });
    if(isBool){
      listContents.remove(listContents.first);
    }
  ...
  ///文件写入
  Future<bool> _writeCounter2(List<int> content) async {
    try {
      ///新文件需要重新创建
      if(isCreate){
        file = await getLocalFile(_fileName!);
