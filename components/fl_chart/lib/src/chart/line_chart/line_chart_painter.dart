...
class LineChartPainter extends AxisChartPainter<LineChartData> {
  ...
  void drawBetweenBarsArea(
    ...
    for (var i = 0; i < fromBarSplitLines.length; i++) {
      ...
      final bottom = min(fromBarData.mostBottomSpot.y, toBarData.mostBottomSpot.y);
  ...
  void drawTouchedSpotsIndicator(
    ...
    for (final info in lineIndexDrawingInfo) {
      ...
      /// horizontalFixed 属性设置为 true 才去绘制另外一条线
      final horizontalFixed = data.lineTouchData.touchTooltipData.horizontalFixed;
      ...
      if (showingDots || horizontalFixed) {
      ...
      /// horizontalFixed 属性设置为 true 才去绘制另外一条线
      if (horizontalFixed) {
        final start = Offset(touchedSpot.dx, -data.lineTouchData.touchTooltipData.tooltipMargin);
        final end = Offset(touchedSpot.dx, touchedSpot.dy - dotHeight / 2);
      ...
      for (final spot in barData.spots) {
        if (barData.aboveBarData.spotsLine.checkToShowSpotLine(spot)) {
          ...
          _barAreaLinesPaint
            ...
            ..strokeWidth = barData.aboveBarData.spotsLine.flLineStyle.strokeWidth
            ..transparentIfWidthIsZero();
