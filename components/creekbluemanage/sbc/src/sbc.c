...
__used static void sbc_decoder_init(struct sbc_decoder_state *state,
...
__used static SBC_ALWAYS_INLINE int16_t sbc_clip16(int32_t s)
{
	if (s > 0x7FFF)
		return 0x7FFF;
	else if (s < -0x8000)
...
static uint8_t sbc_static_init_buffer[sizeof(struct sbc_priv) + SBC_ALIGN_MASK] = {0};
...
int sbc_init(sbc_t *sbc, unsigned long flags)
{
	...
	memset(sbc_static_init_buffer, 0, sizeof(struct sbc_priv) + SBC_ALIGN_MASK);
			...
			void *output, size_t output_len, ssize_t *written)
{
	...
	if (!priv->init) {
		...
		printf("codesize = %d",priv->frame.codesize);
	...
	if (!output || output_len < priv->frame.length)
	{
		...
		printf("priv->frame.length = %d",priv->frame.length);
		return -ENOSPC;
	...
	for (ch = 0; ch < channels; ch++) {
		for (sb = 0; sb < subbands; sb++) {
			...
			for (blk = 0; blk < blocks; blk++) {
				int32_t tmp = fabsc(sb_sample_f[blk][ch][sb]);
				if (tmp != 0)
					x |= tmp - 1;
			}
	...
	int blocks, int subbands)
{
	...
	y = 1 << SCALE_OUT_BITS;
	for (blk = 0; blk < blocks; blk++) {
		tmp0 = fabsc(sb_sample_f[blk][0][sb]);
		tmp1 = fabsc(sb_sample_f[blk][1][sb]);
		if (tmp0 != 0)
			x |= tmp0 - 1;
		if (tmp1 != 0)
			y |= tmp1 - 1;
	...
	while (--sb >= 0) {
		...
		y = 1 << SCALE_OUT_BITS;
		for (blk = 0; blk < blocks; blk++) {
			tmp0 = fabsc(sb_sample_j[blk][0]);
			tmp1 = fabsc(sb_sample_j[blk][1]);
			if (tmp0 != 0)
				x |= tmp0 - 1;
			if (tmp1 != 0)
				y |= tmp1 - 1;
...
ssize_t sbc_decoder_decode(sbc_t *sbc, const char *input, size_t input_len,
			char *output, size_t output_len, size_t *written)
{
	return sbc_decode(sbc, input, input_len,output, output_len, written);
}

static bool m_decode_sbc_init = false;
static bool m_encode_sbc_init = false;
static sbc_t m_decode_sbc;
static sbc_t m_encode_sbc;


void my_sbc_init()
{
    m_decode_sbc_init = false;
    m_encode_sbc_init = false;
    memset(&m_decode_sbc,0,sizeof(m_decode_sbc));
    memset(&m_encode_sbc,0,sizeof(m_encode_sbc));

}

ssize_t my_decoder_decode(const char *input, size_t input_len,
			char *output, size_t output_len, size_t *written)
{

    if(m_decode_sbc_init == false)
    {
          sbc_init_msbc(&m_decode_sbc, 0L);
          m_decode_sbc.endian = SBC_LE;
          m_decode_sbc_init = true;
    }

	return sbc_decode(&m_decode_sbc, input, input_len,output, output_len, written);
}
...
ssize_t my_encoder_encode(const char *input, size_t input_len,
             char *output, size_t output_len, ssize_t *written)
{
    if(m_encode_sbc_init == false)
    {
          sbc_init_msbc(&m_encode_sbc, 0L);
          m_encode_sbc.endian = SBC_LE;
          m_encode_sbc_init = true;
    }
	return sbc_encode(&m_encode_sbc, input, input_len, output, output_len, written);
}

void sbc_encoder_uninit(sbc_t *sbc)
{
	if(sbc_is_init == false)
	{
		return;
	}
	sbc_is_init = false;
