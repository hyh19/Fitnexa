...
class ByteArray extends Iterable<int> {
  ...
  factory ByteArray.char(String string, {int length = 1}) {
    ...
    return ByteArray._fromByteData(byteData);
  }

  factory ByteArray.bool(bool value) =>
      ByteArray._fromByteData(ByteData(1)..setUint8(0, value ? 1 : 0));
...
extension Uint8ListEx on List<int> {
  Uint8List get aligned4Byte {
    int length = this.length;

    // 计算需要补0的字节数
    int padding = (4 - (length % 4)) % 4;

    // 创建一个新的 Uint8List，长度为原始长度加上补0的长度
    Uint8List alignedList = Uint8List(length + padding);

    // 将原始数据复制到新的 Uint8List 中
    alignedList.setAll(0, this);
