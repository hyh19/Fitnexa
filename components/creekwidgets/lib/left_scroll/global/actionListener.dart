...
class GlobalLeftScroll {
  ...
  needCloseSelfRowOfTag(LeftScrollCloseTag? tag, Key? key) {
    if (tag == null) return;
    if (map[tag] == null) return;
    for (var otherKey in map[tag]!.keys) {
      if (otherKey == key){
  ...
  needOPenRowOfTag(LeftScrollCloseTag? tag, Key? key) {
    if (tag == null) return;
    if (map[tag] == null) return;
    for (var otherKey in map[tag]!.keys) {
      if (otherKey != key) continue;
      if (map[tag]![otherKey]!.value == LeftScrollStatus.close) {
        map[tag]![otherKey]!.value = LeftScrollStatus.open;
      }
    }
  }
