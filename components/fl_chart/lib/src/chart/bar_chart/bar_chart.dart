...
class _BarChartState extends AnimatedWidgetBaseState<BarChart> {
  ...
  BarChartData _withTouchedIndicators(BarChartData barChartData) {
    ...
    for (var i = 0; i < barChartData.barGroups.length; i++) {
      ...
      newGroups.add(group.copyWith(showingTooltipIndicators: _showingTouchedTooltips[i]));
    }
  ...
  BarChartData _getData() {
    if (dataHashCode != widget.data.hashCode) {
      _showingTouchedTooltips.clear();
    }

    dataHashCode = widget.data.hashCode;
  ...
  void _handleBuiltInTouch(FlTouchEvent event, BarTouchResponse? touchResponse) {
    ...
    if (!event.isInterestedForInteractions || touchResponse == null || touchResponse.spot == null) {
      /// 什么时候才会清除tooltip
      /// 1、horizontalFixed 属性设置为 false
      /// 2、fitInsideVertically 属性设置为 true
      /// 3、再次点击已选中的柱状图
      /// 2、没有点击到柱状图或折线图 touchResponse 不为null spot 为 null

      if (event is FlTapUpEvent || event is FlPanEndEvent) {
        final horizontalFixed = widget.data.barTouchData.touchTooltipData.horizontalFixed;
        final fitInsideVertically = widget.data.barTouchData.touchTooltipData.fitInsideVertically;

        if (touchResponse?.spot == null || fitInsideVertically || !horizontalFixed) {
          setState(_showingTouchedTooltips.clear);
        }

        if (touchResponse?.spot?.touchedBarGroup.x == preTouchX) {
          preTouchX = -1;
          setState(_showingTouchedTooltips.clear);
        } else {
          preTouchX = touchResponse?.spot?.touchedBarGroup.x ?? -1;
        }
      }
      return;
    }
