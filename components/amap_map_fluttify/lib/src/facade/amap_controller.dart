...
class AmapController with WidgetsBindingObserver, IMapController {
  ...
  final imageConfiguration = ImageConfiguration(devicePixelRatio: ui.window.devicePixelRatio);
  ...
  Future<void> showMyLocation(MyLocationOption option) async {
    ...
    await platform(
      android: (pool) async {
        final map = await androidController!.getMap();
        final locationStyle = await com_amap_api_maps_model_MyLocationStyle.create__();
        ...
        if (option.show) {
          switch (option.myLocationType) {
            ...
            case MyLocationType.FollowNoCenter:
              await locationStyle.myLocationType(
                com_amap_api_maps_model_MyLocationStyle.LOCATION_TYPE_FOLLOW_NO_CENTER,
              );
              break;
            case MyLocationType.Rotate:
              await locationStyle.myLocationType(
                com_amap_api_maps_model_MyLocationStyle.LOCATION_TYPE_MAP_ROTATE,
              );
              break;
          }
          // 定位间隔
          ...
          if (option.iconProvider != null) {
            final imageData = await option.iconProvider!.toImageData(imageConfiguration);
            final bitmap = await android_graphics_Bitmap.create(imageData);
            final bitmapDescriptor =
            await com_amap_api_maps_model_BitmapDescriptorFactory.fromBitmap(bitmap);
          ...
          // 边框颜色
          await locationStyle.strokeColor(Int32List.fromList([option.strokeColor.value])[0]);
          // 填充颜色
          await locationStyle.radiusFillColor(Int32List.fromList([option.fillColor.value])[0]);
          // 边框宽度
      ...
      ios: (pool) async {
        ...
        if (option.show) {
          if (option.interval != Duration.zero) {
            ...
            _locateSubscription = Stream.periodic(option.interval, (_) => _).listen((_) {
              ...
              } else if (option.myLocationType == MyLocationType.FollowNoCenter) {
                iosController!.setUserTrackingMode_animated(
  ...
  Future<void> setZoomLevel(double level, {bool animated = true}) async {
    ...
    await platform(
      android: (pool) async {
        final map = await androidController!.getMap();
        final cameraUpdate = await com_amap_api_maps_CameraUpdateFactory.zoomTo(level);
        if (animated) {
  ...
  Future<void> zoomIn({bool animated = true}) async {
    await platform(
      android: (pool) async {
        final map = await androidController!.getMap();
        final cameraUpdate = await com_amap_api_maps_CameraUpdateFactory.zoomIn();
        if (animated) {
          await map!.animateCamera__com_amap_api_maps_CameraUpdate(cameraUpdate!);
        } else {
          await map!.moveCamera(cameraUpdate!);
  ...
  Future<void> zoomOut({bool animated = true}) async {
    await platform(
      android: (pool) async {
        final map = await androidController!.getMap();
        final cameraUpdate = await com_amap_api_maps_CameraUpdateFactory.zoomOut();
        if (animated) {
          await map!.animateCamera__com_amap_api_maps_CameraUpdate(cameraUpdate!);
        } else {
          await map!.moveCamera(cameraUpdate!);
  ...
  Future<void> setCenterCoordinate(
    ...
    await platform(
      android: (pool) async {
        ...
        final latLng = await com_amap_api_maps_model_LatLng.create__double__double(lat, lng);
        ...
        final cameraUpdate =
        await com_amap_api_maps_CameraUpdateFactory.newCameraPosition(cameraPosition);
  ...
  Future<IMarker> addMarker(MarkerOption option) {
    ...
    return platform(
      android: (pool) async {
        ...
        // marker经纬度
        final latLng =
        await com_amap_api_maps_model_LatLng.create__double__double(latitude, longitude);

        // marker配置
        final markerOption = await com_amap_api_maps_model_MarkerOptions.create__();
        ...
        if (option.iconsProvider != null && option.iconsProvider!.isNotEmpty) {
          ...
          final icon =
          await com_amap_api_maps_model_BitmapDescriptorFactory_Batch.fromBitmap_batch(bitmap!);
          await markerOption
              .icons(icon.whereType<com_amap_api_maps_model_BitmapDescriptor>().toList());
        ...
        // 普通图片
          ...
          Uint8List iconData = await option.iconProvider!.toImageData(imageConfiguration);

          final bitmap = await android_graphics_Bitmap.create(iconData);
          final icon = await com_amap_api_maps_model_BitmapDescriptorFactory.fromBitmap(bitmap);
      ...
      ios: (pool) async {
        ...
        final coordinate = await CLLocationCoordinate2D.create(latitude, longitude);
        ...
        // widget as marker
          ...
          List<Uint8List> iconData = (await state!.widgetToImageData([option.widget!]))!;
  ...
  Future<List<IMarker>> addMarkers(List<MarkerOption> options) async {
    ...
    final infoWindowEnabledBatch = options.map((it) => it.infoWindowEnabled).toList();
    final objectBatch = options.map((it) => it.object ?? '').toList();
    final iconDataBatch = <Uint8List>[
      ...await Future.wait([
        for (final option in options)
          if (option.iconProvider != null) option.iconProvider!.toImageData(imageConfiguration)
      ]),
      ...?await state!.widgetToImageData(
          options.where((element) => element.widget != null).map((e) => e.widget!).toList()),
    ];
    ...
    return platform(
      android: (pool) async {
        ...
        final map = await androidController!.getMap();
        final latLngBatch =
        await com_amap_api_maps_model_LatLng.create_batch__double__double(latBatch, lngBatch);
        // marker配置
        final markerOptionBatch =
        await com_amap_api_maps_model_MarkerOptions.create_batch__(options.length);
        // 添加经纬度
        ...
        if (iconDataBatch.isNotEmpty) {
          final bitmapBatch = await android_graphics_Bitmap.create_batch(iconDataBatch);
          final iconBatch = await com_amap_api_maps_model_BitmapDescriptorFactory_Batch
              .fromBitmap_batch(bitmapBatch!);
          await markerOptionBatch
              .icon_batch(iconBatch.whereType<com_amap_api_maps_model_BitmapDescriptor>().toList());
          pool
            ..addAll(bitmapBatch)
            ..addAll(iconBatch);
        }
      ...
      ios: (pool) async {
        ...
        // 创建marker
        final annotationBatch = await MAPointAnnotation.create_batch__(options.length);

        // 经纬度列表
        final coordinateBatch = await CLLocationCoordinate2D.create_batch(latBatch, lngBatch);
        // 设置经纬度
        ...
        return [
          for (int i = 0; i < options.length; i++) Marker.ios(annotationBatch[i], iosController)
        ];
      },
    );
  }
  ...
  Future<void> clearMarkers(List<IMarker> markers) async {
    await platform(
      android: (pool) async {
        final markerBatch = markers.cast<Marker>().map((it) => it.androidModel).toList();
        await markerBatch.remove_batch();
      },
      ios: (pool) async {
        final markerBatch = markers.cast<Marker>().map((it) => it.iosModel).toList();
  ...
  Future<LatLng> fromScreenLocation(Point point) async {
    return platform(
      android: (pool) async {
        ...
        final androidPoint = await android_graphics_Point.create(point.x.toInt(), point.y.toInt());
      ...
      ios: (pool) async {
        final cgPoint = await CGPoint.create(point.x.toDouble(), point.y.toDouble());
        final coord2d =
        await iosController!.convertPoint_toCoordinateFromView(cgPoint, iosController!);
  ...
  Future<Point> toScreenLocation(LatLng coordinate) async {
    return platform(
      android: (pool) async {
        ...
        final latLng = await com_amap_api_maps_model_LatLng.create__double__double(
      ...
      ios: (pool) async {
        final latLng =
        await CLLocationCoordinate2D.create(coordinate.latitude, coordinate.longitude);
        final point = await iosController!.convertCoordinate_toPointToView(latLng, iosController!);
  ...
  Future<IPolyline> addPolyline(PolylineOption option) async {
    ...
    final longitudeBatch = option.coordinateList.map((e) => e.longitude).toList();
    final textureData = await option.textureProvider?.toImageData(imageConfiguration);
    return platform(
      android: (pool) async {
        final map = await androidController!.getMap();
        ...
        // 构造折线参数
        final polylineOptions = await com_amap_api_maps_model_PolylineOptions.create__();

        if (option.colorList != null && option.colorList!.isNotEmpty) {
          var colorList = option.colorList!.map((e) => Int32List.fromList([e.value])[0]).toList();
          // 添加线段颜色
          await polylineOptions.colorValues(colorList);
        }

        // 是否是渐变色
        await polylineOptions.useGradient(option.useGradient ?? false);

        // 添加经纬度列表
        await polylineOptions.addAll(latLngList);
        await polylineOptions.width(option.width);
        // 颜色
        await polylineOptions.color(Int32List.fromList([option.strokeColor.value])[0]);
        // 自定义贴图
        if (textureData != null) {
          final bitmap = await android_graphics_Bitmap.create(textureData);
          final texture = await com_amap_api_maps_model_BitmapDescriptorFactory.fromBitmap(bitmap);
        ...
        if (option.lineCapType != null) {
          await polylineOptions.lineCapType(
            com_amap_api_maps_model_PolylineOptions_LineCapType.values[option.lineCapType!.index],
          );
        }
        // 线段连接处样式, 由于两端的枚举顺序是一样的, 所以这里直接从索引获取枚举
        if (option.lineJoinType != null) {
          await polylineOptions.lineJoinType(com_amap_api_maps_model_PolylineOptions_LineJoinType
      ...
      ios: (pool) async {
        ...
        // await CLLocationCoordinate2D.create_batch(latitudeBatch, longitudeBatch);
        //
        // // 构造折线参数
        // final polyline =
        // await MAPolyline.polylineWithCoordinates_count(latLngList, latLngList.length);
        //
        // // 宽度和颜色需要设置到STACK里去
        ...
        final multiPolyline = await MAMultiPolyline.create__();

        // 构造折线点
        List<CLLocationCoordinate2D> latLngList = await CLLocationCoordinate2D.create_batch(latitudeBatch, longitudeBatch);
        await multiPolyline.setPolylineWithCoordinates_count(latLngList, latLngList.length);

        // 宽度
        final pixelRatio = MediaQuery.of(state!.context).devicePixelRatio;
        await multiPolyline.addJsonableProperty__(1, option.width / pixelRatio);

        // 颜色
        await multiPolyline.addJsonableProperty__(2, option.strokeColor.value);

        // 设置图片
        if (textureData != null) {
          final texture = await UIImage.create(textureData);
          await multiPolyline.addProperty__(3, texture);
          pool.add(texture);
        }

        // 线段始末端样式, 由于两端的枚举顺序是一样的, 所以这里直接从索引获取枚举
        if (option.lineCapType != null) {
          await multiPolyline.addJsonableProperty__(4, option.lineCapType!.index);
        }

        // 线段连接处样式, 由于两端的枚举顺序是一样的, 所以这里直接从索引获取枚举
        if (option.lineJoinType != null) {
          await multiPolyline.addJsonableProperty__(5, option.lineJoinType!.index);
        }

        // 是否虚线
        if (option.dashType != null) {
          await multiPolyline.addJsonableProperty__(6, option.dashType!.index + 1);
        }

        // 是否渐变色
        if (option.useGradient != null) {
          await multiPolyline.addJsonableProperty__(7, option.useGradient!);
        }

        // 线段颜色数据
        if (option.colorList != null) {
          var colors = option.colorList!.map((color) => color.value).toList();
          await multiPolyline.addJsonableProperty__(8, colors);

          var index = 0;
          var colorList = option.colorList!.map((e) => index++).toList();
          await multiPolyline.set_drawStyleIndexes(colorList);
        }

        // 构造多彩线
        final multiColorPolylineRenderer = await MAMultiColoredPolylineRenderer.create__();
        await multiColorPolylineRenderer.initWithMultiPolyline(multiPolyline);

        // 设置参数
        await iosController!.addOverlay(multiPolyline);

        pool.addAll(latLngList);

        return Polyline.ios(multiPolyline, iosController);
      },
    );
  }
  ...
  Future<IPolygon> addPolygon(PolygonOption option) {
    ...
    return platform(
      android: (pool) async {
        ...
        // 构造参数
        final polygonOptions = await com_amap_api_maps_model_PolygonOptions.create__();
        ...
        // 边框颜色
        await polygonOptions.strokeColor(Int32List.fromList([option.strokeColor.value])[0]);
        // 填充颜色
        await polygonOptions.fillColor(Int32List.fromList([option.fillColor.value])[0]);
      ...
      ios: (pool) async {
        ...
        // 构造折线参数
        final polygon = await MAPolygon.polygonWithCoordinates_count(latLngList, latLngList.length);
  ...
  Future<ICircle> addCircle(CircleOption option) {
    return platform(
      android: (pool) async {
        ...
        // 构造参数
        final circleOptions = await com_amap_api_maps_model_CircleOptions.create__();
        ...
        // 边框颜色
        await circleOptions.strokeColor(Int32List.fromList([option.strokeColor.value])[0]);
        // 填充颜色
        await circleOptions.fillColor(Int32List.fromList([option.fillColor.value])[0]);
  ...
  Future<void> setMarkerClickedListener(OnMarkerClicked onMarkerClicked) async {
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnMarkerClickListener.anonymous__();
            ...
            ..mapView_didAnnotationViewTapped = (_, view) async {
              onMarkerClicked(
                Marker.ios(
                  ...
                  MAPointAnnotation()..refId = (await view!.get_annotation())!.refId,
                  iosController,
                ),
              );
            },
  ...
  Future<void> setMarkerDragListener({
    ...
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnMarkerDragListener.anonymous__();
        ...
        await iosController!.set_delegate(
          _iosMapDelegate!
            ..mapView_annotationView_didChangeDragState_fromOldState =
              ...
              final annotation =
              AmapMapFluttifyIOSAs<MAPointAnnotation>(await view!.get_annotation());
              if (newState == MAAnnotationViewDragState.MAAnnotationViewDragStateStarting) {
                await onMarkerDragStart?.call(Marker.ios(annotation, iosController));
              }

              if (newState == MAAnnotationViewDragState.MAAnnotationViewDragStateDragging) {
                await onMarkerDragging?.call(Marker.ios(annotation, iosController));
              }

              if (newState == MAAnnotationViewDragState.MAAnnotationViewDragStateEnding) {
                await onMarkerDragEnd?.call(Marker.ios(annotation, iosController));
              }
            },
        );
      },
  ...
  Future<void> setMapClickedListener(OnMapClicked onMapClick) async {
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnMapClickListener.anonymous__();
  ...
  Future<void> setMapLongPressedListener(OnMapClicked onMapLongPress) async {
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnMapLongClickListener.anonymous__();
  ...
  Future<void> setMapMoveListener({
    ...
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnCameraChangeListener.anonymous__();
        ...
        await iosController!.set_delegate(
          _iosMapDelegate!
            ..mapView_regionWillChangeAnimated_wasUserAction =
              ...
              var latitude = await location?.latitude;
              var longitude = await location?.longitude;
              await onMapMoveStart?.call(MapMove(
                coordinate: LatLng(latitude ?? 0.0, longitude ?? 0),
  ...
  Future<Uint8List> screenShot() async {
    return platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnMapScreenShotListener.anonymous__();
  ...
  Future<void> setMapRegionLimits(LatLng southWest, LatLng northEast) async {
    await platform(
      android: (pool) async {
        ...
        final southWestPoint = await com_amap_api_maps_model_LatLng.create__double__double(
            southWest.latitude, southWest.longitude);
        final northEastPoint = await com_amap_api_maps_model_LatLng.create__double__double(
  ...
  Future<void> setInfoWindowClickListener(
    ...
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnInfoWindowClickListener.anonymous__();
  ...
  Future<IUrlTileOverlay> addUrlTileOverlay(UrlTileOption option) async {
    ...
    return platform(
      android: (pool) async {
        ...
        final options = await com_amap_api_maps_model_TileOverlayOptions.create__();
        final provider =
        await com_amap_api_maps_model_UrlTileProvider_X.create(width, height, urlTemplate);
  ...
  Future<void> showCustomInfoWindow(Marker marker, Widget widget) async {
    ...
    await platform(
      ...
      ios: (pool) async {
        ...
        // 去掉默认的弹窗
        final annotationView = await iosController!.viewForAnnotation(marker.iosModel!);
  ...
  Future<IGroundOverlay> addGroundOverlay(GroundOverlayOption option) async {
    final imageData =
    await option.imageProvider.toImageData(createLocalImageConfiguration(state!.context));
    return platform(
      android: (pool) async {
        final map = await androidController!.getMap();

        final groundOverlayOption = await com_amap_api_maps_model_GroundOverlayOptions.create__();
        ...
        // 创建图片
        final bitmap = await android_graphics_Bitmap.create(imageData);
        final descriptor = await com_amap_api_maps_model_BitmapDescriptorFactory.fromBitmap(bitmap);
      ...
      ios: (pool) async {
        ...
        final bounds = await MACoordinateBoundsMake(northEastPoint, southWestPoint);

        final bitmap = await UIImage.create(imageData);
        final overlay = await MAGroundOverlay.groundOverlayWithBounds_icon(bounds!, bitmap);
  ...
  Future<IHeatmapOverlay> addHeatmapTileOverlay(
    ...
    return platform(
      android: (pool) async {
        ...
        // 创建热力图Provider
        final builder = await com_amap_api_maps_model_HeatmapTileProvider_Builder.create__();
        List<com_amap_api_maps_model_LatLng> latLngList = [];
        for (final latLng in option.coordinateList) {
          latLngList.add(await com_amap_api_maps_model_LatLng.create__double__double(
        ...
        if (option.gradient != null) {
          await builder.gradient(
            await com_amap_api_maps_model_GradientX.create(
              Int32List.fromList(option.gradient!.colors.map((e) => e.value).toList()),
        ...
        // 创建Tile Overlay选项
        final tileOverlayOption = await com_amap_api_maps_model_TileOverlayOptions.create__();
      ...
      ios: (pool) async {
        ...
        if (option.gradient != null) {
          ...
          await gradient.initWithColor_andWithStartPoints(
            <UIColor>[for (final color in option.gradient!.colors) await UIColor.create(color)],
  ...
  Future<void> zoomToSpan(
    ...
    await platform(
      android: (pool) async {
        ...
        // 西南角
        final southWest =
        await com_amap_api_maps_model_LatLng.create__double__double(minLat, minLng);
        // 东北角
        final northEast =
        await com_amap_api_maps_model_LatLng.create__double__double(maxLat, maxLng);
        ...
        final cameraUpdate = await com_amap_api_maps_CameraUpdateFactory.newLatLngBoundsRect(
          rect,
      ...
      ios: (pool) async {
        ...
        // 矩形宽度
        final width = ((await southEastPoint!.get_x())! - (await northWestPoint.get_x())!).abs();
        // 矩形高度
        final height = ((await southEastPoint.get_y())! - (await northWestPoint.get_y())!).abs();
  ...
  Future<void> setCustomMapStyle({
    ...
    if (styleDataPath != null) {
      styleData =
      await rootBundle.load(styleDataPath).then((byteData) => byteData.buffer.asUint8List());
    }
    Uint8List? styleExtra;
    if (styleExtraPath != null) {
      styleExtra =
      await rootBundle.load(styleExtraPath).then((byteData) => byteData.buffer.asUint8List());
    }
    Uint8List? texture;
    if (texturePath != null) {
      texture =
      await rootBundle.load(texturePath).then((byteData) => byteData.buffer.asUint8List());
    }
    await platform(
      android: (pool) async {
        final map = await androidController!.getMap();
        ...
        // 构造选项
        final option = await com_amap_api_maps_model_CustomMapStyleOptions.create__();
  ...
  Future<void> setMultiPointClickedListener(
    ...
    await platform(
      android: (pool) async {
        ...
        final listener = await com_amap_api_maps_AMap_OnMultiPointClickListener.anonymous__();
      ...
      ios: (pool) async {
        final delegate = await MAMultiPointOverlayRendererDelegate.anonymous__();
  ...
  Future<ISmoothMoveMarker> addSmoothMoveMarker(
    ...
    final iconData =
    await option.iconProvider.toImageData(createLocalImageConfiguration(state!.context));
    return platform(
      android: (pool) async {
        // 获取地图
        final map = await androidController!.getMap();
        ...
        // 动画途经点
        final points = await com_amap_api_maps_model_LatLng.create_batch__double__double(
      ...
      ios: (pool) async {
        ...
        // 动画途经点
        final points = await CLLocationCoordinate2D.create_batch(latitudeBatch, longitudeBatch);
  ...
  Future<IMultiPointOverlay> addMultiPointOverlay(
    ...
    final latitudeBatch = option.pointList.map((it) => it.coordinate.latitude).toList();
    final longitudeBatch = option.pointList.map((it) => it.coordinate.longitude).toList();
    ...
    return platform(
      android: (pool) async {
        ...
        final overlayOptions = await com_amap_api_maps_model_MultiPointOverlayOptions.create__();

        final latLngBatch = await com_amap_api_maps_model_LatLng.create_batch__double__double(
            latitudeBatch, longitudeBatch);
        ...
        final multiPointOverlay = await map!.addMultiPointOverlay(overlayOptions);
      ...
      ios: (pool) async {
        ...
        await pointItemList.addJsonableProperty_batch(1, objectBatch.whereNotNull());
  ...
  Future<void> setBearing(double bearing, {bool animated = true}) async {
    return platform(
      android: (pool) async {
        ...
        final update = await com_amap_api_maps_CameraUpdateFactory.changeBearing(bearing);
        if (animated) {
  ...
  Future<void> setTilt(double tilt, {bool animated = true}) async {
    return platform(
      android: (pool) async {
        ...
        final update = await com_amap_api_maps_CameraUpdateFactory.changeTilt(tilt);
        if (animated) {
          await map!.animateCamera__com_amap_api_maps_CameraUpdate(update!);
        } else {
          await map!.moveCamera(update!);
  ...
  Future<void> setCameraPosition({
    ...
    return platform(
      android: (pool) async {
        ...
        final update =
        await com_amap_api_maps_CameraUpdateFactory.newCameraPosition((await builder.build())!);
        if (animated) {
          await map!
  ...
  Future<List<IPolygon>> addDistrictOutline(
    ...
    final district = await AmapSearch.instance.searchDistrict(districtName, showBoundary: true);
