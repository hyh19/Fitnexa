...
class BarChartPainter extends AxisChartPainter<BarChartData> {
  ...
  void drawBars(
    ...
    for (var i = 0; i < data.barGroups.length; i++) {
      ...
      for (var j = 0; j < barGroup.barRods.length; j++) {
        ...
        // draw Main Rod 当fromY和toY相等时可以绘制
  ...
  void drawTouchTooltip(
    ...
    /// 如果 Tooltip 属性 horizontalFixed
    /// true  位置图表顶部 要确定图表的高度 直接去掉 barTopY 还要注意 tooltipData.direction
    /// false 位置按照原来设置的
    final tempBarTopY = tooltipData.horizontalFixed ? 0 : barTopY;
    final tooltipTop = drawTooltipOnTop
        ? tempBarTopY - tooltipHeight - tooltipData.tooltipMargin
    ...
    /// 要考虑Tooltip边界问题 - 只有 horizontalFixed 属性为 true 才考虑
    /// fitInsideHorizontally 设置为 true 也可以解决
    var tempTooltipLeft = tooltipLeft;
    if (!tooltipData.fitInsideHorizontally && tooltipData.horizontalFixed) {
      if (tooltipLeft < 0) {
        tempTooltipLeft = 0;
      } else if (tooltipLeft + tooltipWidth > viewSize.width) {
        tempTooltipLeft = tooltipLeft - (tooltipLeft + tooltipWidth - viewSize.width);
      }
    }
    ...
    _touchLinePaint
      ..color = tooltipData.touchLineIndicator.color
      ..strokeWidth = tooltipData.touchLineIndicator.strokeWidth
      ..transparentIfWidthIsZero();

    /// 绘制竖线
    if (!tooltipData.fitInsideVertically && tooltipData.horizontalFixed) {
      drawTouchLine(
        canvasWrapper,
        showOnRodData,
        holder,
  ...
  void drawTouchLine(
    ...
    /// 绘制竖线
    /// 注意事项 - 边缘绘制时不再是Tooltip的中心点
    final startX = tooltipLeft + tooltipWidth / 2;
    final startY = tooltipTop + tooltipHeight;
    var start = Offset(startX, startY);
    final middle = Offset(startX, barOffset.dy);
    canvasWrapper.drawLine(start, middle, _touchLinePaint);

    final y = getPixelY(showOnRodData.fromY, viewSize, holder);
    start = Offset(startX, y);
    final end = Offset(startX, viewSize.height);
    canvasWrapper.drawLine(start, end, _touchLinePaint);
  }
  ...
  BarTouchedSpot? handleTouch(
    ...
    for (var i = 0; i < _groupBarsPosition!.length; i++) {
      ...
      for (var j = 0; j < groupBarPos.barsX.length; j++) {
        ...
        bool isYInBarBounds;
        if (isUpward) {
          isYInBarBounds = (touchedPoint.dy <= barBotY + touchExtraThreshold.bottom) &&
