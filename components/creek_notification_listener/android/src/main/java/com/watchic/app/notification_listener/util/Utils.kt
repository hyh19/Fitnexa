...
class Utils {
    ...
    class PromoteServiceConfig {
        ...
        fun save(context: Context) {
            ...
            if((title?:"").isEmpty()){
                Log.d(TAG, "save the promote config isEmpty 不保存")
                LogUtilHandler.getInstance(context)?.sendMessage( "$TAG,save the promote config isEmpty 不保存")
                return
            }
            try {
                context.getSharedPreferences(NotificationListenerPlugin.SHARED_PREFERENCES_KEY, Context.MODE_PRIVATE)
                    .edit()
                    .putString(NotificationListenerPlugin.PROMOTE_SERVICE_ARGS_KEY, str)
                    .apply()
            }catch (e:Exception){
                e.printStackTrace()
            }
        ...
        companion object {
            ...
            fun fromMap(map: Map<*, *>?): PromoteServiceConfig {
             ...
             private fun fromString(str: String, myTitle:String): PromoteServiceConfig {
                val cfg = PromoteServiceConfig()
                if(str.isEmpty() || str =="{}"){
                    cfg.title== myTitle
                ...
                map.let { m ->
                    try {
                        try {
                            m["title"].apply { cfg.title = this as String? }
                        }catch (e:Exception){
                            cfg.title== myTitle
                        }


                       // m["title"].let { cfg.title = "Watch App" }
                        try {
                            m["subTitle"].apply { cfg.subTitle = this as String? }
                        }catch (e:Exception){
                            cfg.subTitle = " "
                        }
                       // cfg.subTitle = " "
                       ...
                       // m["subTitle"].let { cfg.subTitle = it as String? }
                        try {
                            m["description"].apply { cfg.description = this as String? }
                        }catch (e:Exception){
                            cfg.description = " "
                        }

                        try {
                             m["showWhen"].apply { cfg.showWhen = this as Boolean? }
                         }catch (e: Exception){
                            Log.w("lxk","Exception--${e.printStackTrace()}")
                             cfg.showWhen = false
                        }

                        m["foreground"].apply { cfg.foreground = this as Boolean? }
            ...
            fun load(context: Context): PromoteServiceConfig {
                ...
                str = context.getSharedPreferences(NotificationListenerPlugin.REMOTE_SHARED_PREFERENCES_KEY, Context.MODE_PRIVATE).getString(NotificationListenerPlugin.REMOTE_PROMOTE_SERVICE_ARGS_KEY, "{}")

                Log.d(TAG, "load the promote config: $str")
                LogUtilHandler.getInstance(context)?.sendMessage("lxk______load the promote config: $str\"")
                if(str.isNullOrEmpty() || str=="{}"){
                    str = context.getSharedPreferences(NotificationListenerPlugin.SHARED_PREFERENCES_KEY, Context.MODE_PRIVATE).getString(NotificationListenerPlugin.PROMOTE_SERVICE_ARGS_KEY, "{}")

                    Log.d(TAG, "load the REMOTE_PROMOTE_SERVICE_ARGS_KEY promote config: $str")
                    LogUtilHandler.getInstance(context)?.sendMessage("lxk______load the REMOTE_PROMOTE_SERVICE_ARGS_KEY promote config: $str")
                    if(str.isNullOrEmpty() || str=="{}"){
