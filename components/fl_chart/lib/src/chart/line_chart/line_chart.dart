...
class _LineChartState extends AnimatedWidgetBaseState<LineChart> {
  ...
  void _handleBuiltInTouch(
    ...
    if (!event.isInterestedForInteractions ||
      ...
      /// 以下代码控制Tooltip是否消失
      final horizontalFixed = widget.data.lineTouchData.touchTooltipData.horizontalFixed;
      final fitInsideVertically = widget.data.lineTouchData.touchTooltipData.fitInsideVertically;

      if (touchResponse?.lineBarSpots == null || fitInsideVertically || !horizontalFixed) {
        setState(() {
      ...
      if (_showingTouchedIndicators.isNotEmpty) {
        var touchIndexs = _showingTouchedIndicators.values.first;
        if (touchIndexs.isNotEmpty) {
          if (touchIndexs.first == preTouchX) {
            preTouchX = -1;
            setState(() {
              _showingTouchedTooltips.clear();
              _showingTouchedIndicators.clear();
