...
#ifdef __GNUC__
//#define __attribute__((aligned(1 << (SBC_ALIGN_BITS))))
#else
#define SBC_ALIGNED
#endif
...
*/
int sbc_six_one_init(sbc_t *sbc, unsigned long flags);
/**@brief 压缩比：8:1初始化
 *@param[in] sbc    创建sbc属性
 *@param[in] flag:
...
*@retval -ENOMEM 		动态申请内存指针为空
*/
int sbc_eight_one_init(sbc_t *sbc, unsigned long flags);
